...
- [ ] **T-1** Handler-тесты для ChatWS access-control (личный/командный/капитанский — сейчас только service_test).
- [ ] **T-2** Тест TTL-семантики `GetOrSet` vs `GetOrSetWithCtx` (ttl=0 → дефолт) — зафиксировать контракт.
- [ ] **T-3** Негативный тест `SubmitCode` при сбое загрузки team_id (запрос должен падать, не писать team_id=0).
- [ ] **T-4** E2E: сценарии с двумя пользователями — личный чат, командный чат, совместная игра (расширение Playwright).
