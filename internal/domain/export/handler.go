...
func (h *ExportHandler) ExportTeamResultsCSV(c *gin.Context) {
	...
	// M1 (PASS-5): экспорт ЧУЖИХ команд требует модератора (CanModerateGame) —
	// IsUserManager пропускает observer (read-only), который не должен видеть
	// данные всех команд (утечка, как в ExportResultsCSV/Excel).
	isManager := false
	if mgr, mgrErr := h.coAuthorSvc.CanModerateGame(c.Request.Context(), gameID, userID); mgrErr != nil {
