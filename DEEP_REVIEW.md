...
#### 1. Платёжный вебхук заблокирован CSRF → платежи ЮKassa никогда не подтверждаются
...
- **Фикс**: добавить `/payments/webhook` в skip-list CSRF (вебхук уже защищён IP-allowlist + подтверждением статуса через API) + регрессионный тест, что вебхук без токена доходит до handler.

#### 2. IDOR: список комнат любой игры без проверки прав
...
- **Фикс**: добавить `gameManager` к маршруту (как для `ListLogs`).
...
1. **`/payments/webhook` → в skip-list CSRF** — без этого платёжный функционал мёртв (подтверждено 403).
2. **`GameRooms` → под `gameManager`** — закрыть IDOR (подтверждено 200 на чужую игру).
3. **Закрывать redis-клиент при недоступном Valkey** — убрать вечные goroutine tryDial (найдено pprof).
4. **Атомарность команды/турнира/лимита команд** — 3 data-integrity race'а.
