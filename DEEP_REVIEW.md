...
- Co-author список гейтится IsUserManager; VK OAuth email — log-warning при линковке
...
| Пункт | Статус |
|---|---|
| **U-3** FOUC списка игр для card-пользователей | JS-fetch определяет вид (~100мс); server-side требует внедрения userSvc в GameHandler |
| **S-L2** VK email не верифицирован | Логируется; полная политика линковки — продуктовое решение |
| **C-M10** SnapshotDispatcher generation | Безвредный лишний пересчёт (идемпотентный) |
| **P-M4** ListByGamePaginated (Count+Preload) | Уже пагинировано; 3 запроса на страницу приемлемо |
